module LambdaEvaluationTests
    ...
    //Тесты на поиск свободных переменных
    [<Test>]
    let ``findFV (λx.(λx.x) x) should be []`` () = 
        findFV (Abstr("x", App(Abstr("x", Variable("x")), Variable("x")))) |> should equal []
    
    [<Test>]
    let ``findFV (λx.(λx.x y) x) should be ["y"]`` () = 
        findFV (Abstr("x", App(Abstr("x", App(Variable("x"), Variable("y"))), Variable("x")))) |> should equal ["y"]
    
    [<Test>]
    let ``findFV (λx.(λx.x y) x z) should be ["y"; "z"]`` () = 
        findFV (Abstr("x", App(App(Abstr("x", App(Variable("x"), Variable("y"))), Variable("x")), Variable("z")))) |> should equal ["y"; "z"]

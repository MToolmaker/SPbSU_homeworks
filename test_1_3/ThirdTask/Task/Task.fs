module TaskModule
    type Hashtable<'TKey, 'TValue when 'TValue : equality>(size : int, getHash : 'TKey -> int) =
        ...
        member __.Contains(key : 'TKey) (value : 'TValue) = List.exists (fun x -> x = value) array.[(getHash key) % size |> abs]

        member __.Delete(key : 'TKey) (value : 'TValue) =
            array.[(getHash key) % size |> abs] <- List.filter (fun x -> x <> value) array.[(getHash key) % size |> abs]
